...
namespace Orion.ApiClientLight
{
    public class JsonApiClientLight : BaseApiClientLight, IJsonApiClientLight
    {
        ...
        protected virtual string CreateUrlParameters(object data)
        {
            ...
            {
                var dictionnary = (IDictionary<string, object>) data;
                foreach (var keyPair in dictionnary)
                {
                    if (keyPair.Value.GetType().GetTypeInfo().IsPrimitive)
                    {
                        parameters.Add(keyPair.Key, keyPair.Value.ToString());
                    }
                    else {
                        parameters.Add(
                            keyPair.Key,
                            JsonConvert.SerializeObject(keyPair.Value, CreateJsonSerializerSettings()));
                    }
                }
            }
            else {
                ...
                foreach (var propertyInfo in type.GetTypeInfo().DeclaredProperties)
                {
                    ...
                    if (value.GetType().GetTypeInfo().IsPrimitive) parameters.Add(propertyInfo.Name, value.ToString());
                    else parameters.Add(propertyInfo.Name, JsonConvert.SerializeObject(value, CreateJsonSerializerSettings()));
                }
            }
